...
def execute_auto_literature_review(
    ...
    *,
    ...
    run_id = ledger.create_auto_research_run(intent_id=intent_id, stage="search_strategy")

    def record_failure(error: LLMRetryExhausted, stage: str, context: dict[str, Any] | None = None) -> None:
    ...
    try:
        ...
        ledger.update_auto_research_run(run_id, stage="abstract_screening")
        def retrying_abstract_reviewer(prompt: str) -> str:
            try:
                ...
                raise
        outcome = run_profile(ledger, profile, "auto", reviewer=retrying_abstract_reviewer)
        ...
        try:
            synthesis, attempts = call_with_retry(
                synthesis_drafter, literature_synthesis_prompt(profile, run_for_report, processed),
        ...
        ledger.update_auto_research_run(run_id, status="completed", stage="completed")
        complete_intent(
            ledger, intent_event,
